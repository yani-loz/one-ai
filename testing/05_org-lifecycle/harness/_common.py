"""
Shared harness helpers for the Org Lifecycle target (PC-03a) — the platform org-lifecycle
surface: GET /platform/orgs/{id}, PATCH .../status, PATCH .../legal-hold, and the
suspend-blocks-login gate on /auth/login + /auth/refresh (while /auth/me stays open).
...
`async def main(): ...` + `asyncio.run(main())`, and is run with:

    docker compose exec -T backend python - < testing/05_org-lifecycle/harness/<script>.py
...
Isolation rules (the DB is persistent + shared):
  - Provision FRESH, run-stamped orgs + unique emails via `stamp()`; never collide.
  - NEVER mutate the demo platform admin (`super@ethera.ai`).
  - **NEVER PATCH the status of the `demo`/`globex` orgs (or any org you didn't onboard).**
    Suspending them blocks their company logins and breaks the demo. Suspend ONLY your own
    fresh run-stamped orgs.

Key facts: tenant key is org_id; RLS is DEFINED BUT INERT (superuser bypass) → the JWT secret
is the single isolation layer; the dev JWT secret is the forgeable default (DEV_SECRET).
Two auth domains by audience (company vs platform). Suspend gate: ONLY `suspended` blocks
login+refresh (403, raised AFTER the credential check → no enumeration oracle); `onboarding`
and `offboarded` are allowed (offboarded cutoff is deferred to PC-06); `/auth/me` is ungated.
"""
...
# Lifecycle status values (match OrganizationStatus enum / the DB CHECK).
...
def _client(timeout: float = 30) -> httpx.AsyncClient:
    """Async client bound to the live server."""
...
async def onboard_org(
    c: httpx.AsyncClient, plat_token: str, *, name: str, slug: str, admin_email: str,
    admin_name: str = "Org Admin", admin_pw: str = DEFAULT_PW,
) -> httpx.Response:
    """Onboard a new org + its first company_admin via POST /platform/orgs. Raw response."""
    return await c.post(
        "/platform/orgs", headers=bearer(plat_token),
        json={"org_name": name, "org_slug": slug, "admin_email": admin_email,
...
async def provision_company(
    ...
    Returns: org_id, slug, admin_email, admin_access, admin_refresh — a complete company
    identity (incl. a PRE-SUSPENSION token pair) for the suspend-gate tests. Raises on
    any non-2xx. The returned org is YOURS — it is safe to suspend/offboard it.
    """
    slug = f"{prefix}-{stamp()}"
    admin_email = f"admin-{slug}@oneai.dev"
    onboarded = await onboard_org(c, plat_token, name=f"Org {slug}", slug=slug, admin_email=admin_email)
    ...
    return {
        "org_id": org_id, "slug": slug, "admin_email": admin_email,
        "admin_access": access, "admin_refresh": refresh,
    }


# — Lifecycle endpoints under test (PC-03a) —

async def get_org_detail(c: httpx.AsyncClient, plat_token: str, org_id: str) -> httpx.Response:
    """GET /platform/orgs/{org_id} — one org's lifecycle detail (7 metadata fields)."""
    return await c.get(f"/platform/orgs/{org_id}", headers=bearer(plat_token))


async def patch_status(
    c: httpx.AsyncClient, plat_token: str, org_id: str, status: str
) -> httpx.Response:
    """PATCH /platform/orgs/{org_id}/status — set lifecycle status. Returns raw response.

    ⚠️ Only call on an org YOU onboarded — never demo/globex."""
    return await c.patch(
        f"/platform/orgs/{org_id}/status", headers=bearer(plat_token), json={"status": status}
    )


async def patch_legal_hold(
    c: httpx.AsyncClient, plat_token: str, org_id: str, legal_hold: bool
) -> httpx.Response:
    """PATCH /platform/orgs/{org_id}/legal-hold — set/clear legal hold. Returns raw response."""
    return await c.patch(
        f"/platform/orgs/{org_id}/legal-hold", headers=bearer(plat_token),
...
def forge_company_token(
    *, sub: str, org_id: str | None, role: str = "company_admin", ttl_min: int = 15,
    ...
    company-aud token whose `sub` is a REAL platform admin id."""
...
def forge_platform_token(
    *, sub: str | None = None, ttl_min: int = 15, secret: str = DEV_SECRET,
    ...
    the unknown-admin path WITHOUT touching the real demo admin; with the dev secret it is a
    real forged-token capability (suspend/legal-hold any org)."""
