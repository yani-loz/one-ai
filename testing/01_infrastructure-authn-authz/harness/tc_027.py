...
# ============================ TC-IA-027 test logic ============================
# A correctly-signed but EXPIRED company token on /auth/me must be 401:
# jwt.decode raises ExpiredSignatureError (ordered first, tokens.py:85) ->
# TokenExpiredError -> 401. Confirms audit section 5 fix #4 (exp enforced).

async def main() -> None:
    async with _client() as c:
        expired = forge_company_token(
            sub=str(uuid4()), org_id=str(uuid4()), role="company_admin", expired=True
        )
        print("[forge] expired company token (iat/exp in the past, valid signature)")

        me = await c.get("/auth/me", headers=bearer(expired))
        print(f"[attack] GET /auth/me (expired token) -> {me.status_code} {me.text}")

        u = await c.get("/users", headers=bearer(expired))
        print(f"[attack] GET /users (expired token) -> {u.status_code} {u.text}")
