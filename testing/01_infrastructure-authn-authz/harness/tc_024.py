...
# ============================ TC-IA-024 test logic ============================
# NO Authorization header on /auth/me, /users, /platform/orgs must be 401 (NOT 403):
# HTTPBearer(auto_error=False) -> credentials is None -> raise TokenInvalidError (401),
# dependencies.py:51,84-85,114-115. Hit the GET variants so no body is involved.

async def main() -> None:
    async with _client() as c:
        me = await c.get("/auth/me")
        print(f"[attack] GET /auth/me (no auth header) -> {me.status_code} {me.text}")

        u = await c.get("/users")
        print(f"[attack] GET /users (no auth header) -> {u.status_code} {u.text}")

        o = await c.get("/platform/orgs")
        print(f"[attack] GET /platform/orgs (no auth header) -> {o.status_code} {o.text}")
