...
async def main() -> None:
    s = stamp()
    slug = f"authn011-{s}"
    admin_email = f"admin-authn011-{s}@oneai.dev"

    async with _client() as c:
        plat = await platform_login(c)
        ob = await onboard_org(
            c, plat, name=f"authn011 co-{s}", slug=slug, admin_email=admin_email,
            admin_name="Authn011 Admin",
        )
        print(f"== onboard == {ob.status_code} admin={admin_email}")

        sanity = await login(c, admin_email, DEFAULT_PW)
        print(f"== sanity login (correct creds) == {sanity.status_code} (account is real)")

        r = await login(c, admin_email, WRONG_PW)
        body = r.json()
        print(f"== POST /auth/login (correct email, WRONG password) == {r.status_code}")
        print(f"body: {body}")
        print(f"body keys: {sorted(body.keys())}")
        print(f"detail == 'Invalid email or password.': "
              f"{body.get('detail') == 'Invalid email or password.'}")

        ok = (
            sanity.status_code == 200
            and r.status_code == 401
            and list(body.keys()) == ["detail"]
            and body.get("detail") == "Invalid email or password."
        )
