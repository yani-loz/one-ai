...
# ======================= TC-IA-045 test logic =======================
# Deactivated user's REFRESH token -> 401 (consume revokes, then get_by_subject_id finds
# inactive -> InvalidCredentials). Target is a MEMBER so the last-admin guard is inert.
...
async def main() -> None:
    ...
    async with _client() as c:
        ...
        print(f"[step1] create member -> {cu.status_code}")
        ...
        victim_r0 = vl.json()["refresh_token"]
        print(f"[step2] victim login -> 200  R0(prefix)={victim_r0[:12]}...")

        dl = await c.delete(f"/users/{member_id}", headers=bearer(admin_at))
        print(f"[step3] admin DELETE /users/{{member}} -> {dl.status_code}  body={dl.text!r}")

        rr = await c.post("/auth/refresh", json={"refresh_token": victim_r0})
        print(f"[step4] refresh(victim R0, now deactivated) -> {rr.status_code}  body={rr.text}")

        rr2 = await c.post("/auth/refresh", json={"refresh_token": victim_r0})
        print(f"[step5] refresh(victim R0 again) -> {rr2.status_code}  body={rr2.text}")

        held = dl.status_code == 204 and rr.status_code == 401 and rr2.status_code == 401
        print(f"[verdict] deactivated-refresh HELD={held} (delete==204 & refresh==401 & re-refresh==401)")
