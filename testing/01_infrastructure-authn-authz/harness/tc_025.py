...
# ============================ TC-IA-025 test logic ============================
# An UNSIGNED alg='none' company token on /auth/me must be 401: decode_access_token
# passes algorithms=['HS256'] (tokens.py:80), so PyJWT rejects 'none' as
# InvalidAlgorithmError(InvalidTokenError) -> TokenInvalidError -> 401. Critical if 200.

async def main() -> None:
    async with _client() as c:
        none_token = forge_company_token(
            sub=str(uuid4()), org_id=str(uuid4()), role="company_admin", alg="none"
        )
        print(f"[forge] alg=none token -> {none_token[:60]}...")

        me = await c.get("/auth/me", headers=bearer(none_token))
        print(f"[attack] GET /auth/me (alg=none token) -> {me.status_code} {me.text}")

        u = await c.get("/users", headers=bearer(none_token))
        print(f"[attack] GET /users (alg=none token) -> {u.status_code} {u.text}")
