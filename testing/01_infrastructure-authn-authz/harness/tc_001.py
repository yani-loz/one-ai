...
# ─────────────────────────────────────────────────────────────────────────────
# TC-IA-001 — /health reports DB reachable (Positive). Sev Info.
# Hypothesis: GET /health returns 200 with database=="reachable" after a real
# SELECT 1 round-trip. A break = non-200, missing/!="reachable" database field,
# or the DB being unreachable while the probe still claims 200.
# ─────────────────────────────────────────────────────────────────────────────
...
async def main() -> None:
    async with _client() as c:
        ...
        try:
            body = r.json()
        except Exception as exc:  # noqa: BLE001 — evidence path, not control flow
            print("JSON parse error:", repr(exc))
            body = {}
        print("--- assertions ---")
        print("status==200:", r.status_code == 200)
        print("database=='reachable':", body.get("database") == "reachable")
        print("status field=='ok':", body.get("status") == "ok")
