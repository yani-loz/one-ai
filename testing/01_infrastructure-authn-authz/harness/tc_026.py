...
# ============================ TC-IA-026 test logic ============================
# Take a REAL company token and flip the last signature char -> 401:
# jwt.decode signature verification fails -> InvalidSignatureError(InvalidTokenError)
# -> TokenInvalidError -> 401. Critical if a tampered token is accepted (200).
...
async def main() -> None:
    ...
    async with _client() as c:
        ...
        ob = await onboard_org(
            c, plat, name=f"AuthZ Org {s}", slug=f"authz-{s}", admin_email=admin_email
        )
        print(f"[setup] onboard_org -> {ob.status_code}")
        admin_login = await login(c, admin_email, DEFAULT_PW)
        real_token = admin_login.json()["access_token"]
        print(f"[setup] admin login -> {admin_login.status_code}")

        ok = await c.get("/auth/me", headers=bearer(real_token))
        print(f"[control] GET /auth/me (real token) -> {ok.status_code}")

        last = real_token[-1]
        flipped = "A" if last != "A" else "B"
        tampered = real_token[:-1] + flipped
        print(f"[tamper] last sig char {last!r} -> {flipped!r}")

        me = await c.get("/auth/me", headers=bearer(tampered))
        print(f"[attack] GET /auth/me (tampered signature) -> {me.status_code} {me.text}")
