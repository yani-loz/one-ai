...
# ============================ TC-IA-028 test logic ============================
# Tokens missing a required claim (exp / aud / sub) must each be 401:
# decode_access_token passes options={"require": ["exp","aud","sub"]} (tokens.py:82),
# so PyJWT raises MissingRequiredClaimError(InvalidTokenError) -> TokenInvalidError 401.

async def main() -> None:
    async with _client() as c:
        for missing in ("exp", "aud", "sub"):
            tok = forge_company_token(
                sub=str(uuid4()), org_id=str(uuid4()), role="company_admin",
                drop=(missing,),
            )
            me = await c.get("/auth/me", headers=bearer(tok))
            print(f"[attack] GET /auth/me (drop {missing!r}) -> {me.status_code} {me.text}")
