...
# ============================ TC-IA-023 test logic ============================
# A REAL platform token on GET /auth/me and GET /users must be 401:
# get_current_principal -> decode_access_token(aud='company') rejects aud='platform'
# inside the audience check (tokens.py:81). Expected 401, NOT 403 (role gate never hit).

async def main() -> None:
    async with _client() as c:
        platform_token = await platform_login(c)
        print("[setup] platform login -> obtained real platform token (aud=platform)")

        me = await c.get("/auth/me", headers=bearer(platform_token))
        print(f"[attack] GET /auth/me (platform token) -> {me.status_code} {me.text}")

        u = await c.get("/users", headers=bearer(platform_token))
        print(f"[attack] GET /users (platform token) -> {u.status_code} {u.text}")
