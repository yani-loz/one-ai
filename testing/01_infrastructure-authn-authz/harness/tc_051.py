...
# ─────────────────────────────────────────────────────────────────────────────
# TC-IA-051 — last-admin race: concurrent PATCH {role:'member'} on admin1 + admin2
#
# Same non-atomic guard as TC-050, generalized to the PATCH/update_user path
# (user_service.py:81 -> _guard_last_admin:123). Two concurrent demotions each EXCLUDE
# the other admin from count_active_admins, both see count=1, both pass, both set
# role='member', both commit -> 0 active company_admins. A correct atomic guard makes
# exactly ONE patch 409.
# ─────────────────────────────────────────────────────────────────────────────
...
async def main() -> None:
    ...
    async with _client() as c:
        ...
        for i in range(ITERATIONS):
            ...
            onb = await onboard_org(c, plat, name=f"Race051 {tag}", slug=slug, admin_email=a1_email)
            ...
            # ── THE RACE: concurrent demotion of both admins to member ──
            async def demote(uid: str) -> httpx.Response:
                return await c.patch(f"/users/{uid}", headers=bearer(a1_token), json={"role": "member"})

            res = await asyncio.gather(demote(admin1_id), demote(admin2_id), return_exceptions=True)
    ...
    print("=" * 70)
    print(f"TC-IA-051  run={run}  iterations={ITERATIONS}")
    print("=" * 70)
    ...
    print("=" * 70)
    print(
        f"VERDICT: {'GUARD BROKE — last-admin PATCH race fired' if zero_admin_iters else 'no lockout observed'}"
