...
|---|---|---|---|---|
| [CA-01](EPIC-CA-01-user-management-console.md) | Company-admin console — user & access management UI | ✅ Done (pending commit) | `feat/company-admin-console` | _pending_ |
...
|---|---|---|---|
| A | CA-01 | Console shell + user CRUD UI (over the built `/users/*`) + break-glass inbox relocated + home entry point | ✅ Done (pending commit) |
| B | CA-02 | Company-scoped audit view (`GET /audit`, app-layer org filter — `audit_log` is NOT RLS-scoped) + the `identity/company` vs `identity/platform` backend seam | ⏳ Planned |
