...
# ───────────────────────── TC-IA-063 ─────────────────────────
# Role-escalation via request body. UserRole enum is {company_admin, member} ONLY.
# Posting role='platform_admin', 'superadmin', or '' must each yield 422 (enum reject)
# — no path through POST /users can mint a platform admin or an arbitrary role.
# Sev HIGH if any non-enum role is accepted (a created user with that role).
...
async def main() -> None:
    s = stamp()
    org_slug = f"{SUITE}-063-{s}"
    admin_email = f"{SUITE}-063-admin-{s}@example.com"
    async with _client() as c:
        plat = await platform_login(c)
        ob = await onboard_org(c, plat, name=f"IV063 {s}", slug=org_slug, admin_email=admin_email)
        ...
        hostile_roles = ["platform_admin", "superadmin", ""]
        for role in hostile_roles:
            email = f"{SUITE}-063-{abs(hash(role)) % 10000}-{s}@example.com"
            r = await create_user(c, admin_token, email=email, full_name="Escalation Probe",
                                  role=role, password=DEFAULT_PW)
            label = role if role else "<empty>"
            print(f"[role={label!r}] status={r.status_code} body={r.text}")
            if r.status_code == 201:
                print(f"[role={label!r}] *** CREATED *** stored_role={r.json().get('role')}")

        # Control: a valid enum role still works (proves the surface is otherwise live).
        ok = await create_user(c, admin_token, email=f"{SUITE}-063-ok-{s}@example.com",
                               full_name="Valid Member", role="member", password=DEFAULT_PW)
        print(f"[control role='member'] status={ok.status_code} "
              f"stored_role={ok.json().get('role') if ok.status_code == 201 else 'n/a'}")
