...
> **Status as of 2026-09-06 (dated record — the findings below are unchanged):** this audit's central caveat — *"tenant isolation is stubbed, not live-enforced"* (§1, §3 layer table, §6) — is **closed**. RLS is ENFORCED since migration `0009_enforce_rls.py`, which creates the non-bypass runtime roles (`oneai_app`, the write plane RLS enforces against, and `oneai_global`, BYPASSRLS for cross-org/pre-org flows; migration `0019` later adds the SELECT-only `oneai_reader`) and applies `FORCE ROW LEVEL SECURITY`. On the live dev DB 22 tables carry ENABLE + FORCE plus an `org_isolation` policy — measured 2026-09-06 (`docs/audits/2026-09-06_built-vs-docs-map.md` §3). The `X-Org-Id` header is **gone**: `grep -rn "X-Org-Id" backend/app frontend/src` → 0 hits; tenant identity now comes from a verified JWT (`backend/app/identity` ships login / refresh / logout / me on `pyjwt>=2.10`). §6 items **1–3 are therefore closed** (cross-tenant negative tests exist — e.g. `backend/tests/identity/routes/test_erasure_cross_tenant.py` plus the `backend/tests/access/` suite; the `set_config` transaction-pin fix of §7 row 1 is now `scoped_session`, which re-binds the GUC on every transaction, `backend/app/core/database.py:105-124`). **§6 item 4 is still open** — there is no backend Python dependency CVE scan in CI (`grep -n "pip-audit" .github/workflows/ci.yml` → 0 hits). §6 item 5 (production CORS origins review) remains a pre-deploy task; items 6–7 were already closed by this document's own §7 same-day remediation.
