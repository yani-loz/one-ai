...
# ───────────────────────── TC-IA-066 ─────────────────────────
# Field-bounds fuzz on POST /users. Assert NO 500 on any variant.
#   - full_name=""            -> 422 (min_length=1)
#   - full_name len 201       -> 422 (max_length=200)
#   - full_name with NUL byte  -> report (Pydantic 201? or DB rejects? must not be 500)
#   - full_name RTL+emoji      -> report (likely 201, stored literally)
#   - missing required field (no role) -> 422
#   - wrong JSON type (role as number) -> 422
...
async def main() -> None:
    s = stamp()
    org_slug = f"{SUITE}-066-{s}"
    admin_email = f"{SUITE}-066-admin-{s}@example.com"
    async with _client() as c:
        plat = await platform_login(c)
        ob = await onboard_org(c, plat, name=f"IV066 {s}", slug=org_slug, admin_email=admin_email)
        ...
        def body(full_name, role="member", email=None, omit=None):
            b = {
                "email": email or f"{SUITE}-066-{stamp()}@example.com",
                "full_name": full_name,
                "role": role,
                "password": DEFAULT_PW,
            }
        ...
        # 1. empty full_name
        r = await c.post("/users", headers=hdr, json=body(""))
        print(f"[full_name=''] status={r.status_code} body={r.text[:300]}")

        # 2. full_name length 201
        r = await c.post("/users", headers=hdr, json=body("X" * 201))
        print(f"[full_name len=201] status={r.status_code} body={r.text[:300]}")

        # 3. full_name with embedded NUL byte
        nul_name = "Ab\x00cd"
        r = await c.post("/users", headers=hdr, json=body(nul_name))
        print(f"[full_name NUL byte] sent_bytes={nul_name.encode('utf-8').hex()} "
              f"status={r.status_code} body={r.text[:400]}")

        # 4. full_name with RTL override + emoji
        rtl_name = "user‮evil\U0001F608"
        r = await c.post("/users", headers=hdr, json=body(rtl_name))
        got = r.json().get("full_name") if r.status_code == 201 else r.text[:200]
        print(f"[full_name RTL+emoji] sent={rtl_name!r} status={r.status_code} "
              f"stored={got!r} exact={got == rtl_name}")

        # 5. missing required field (role omitted)
        r = await c.post("/users", headers=hdr, json=body("Valid Name", omit="role"))
        print(f"[missing role] status={r.status_code} body={r.text[:300]}")

        # 6. wrong JSON type: role as a number
        bad = body("Valid Name")
        bad["role"] = 12345
        r = await c.post("/users", headers=hdr, json=bad)
        print(f"[role=number] status={r.status_code} body={r.text[:300]}")
