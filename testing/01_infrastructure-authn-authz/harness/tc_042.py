...
# ======================= TC-IA-042 test logic =======================
# Logout is idempotent: same token twice -> 204/204; unknown token -> 204.
...
async def main() -> None:
    ...
    async with _client() as c:
        ...
        print(f"[setup] login -> 200  R0(prefix)={r0[:12]}...")

        lo1 = await c.post("/auth/logout", json={"refresh_token": r0})
        print(f"[step2] logout(R0) -> {lo1.status_code}  body={lo1.text!r}")

        lo2 = await c.post("/auth/logout", json={"refresh_token": r0})
        print(f"[step3] logout(R0 again, already revoked) -> {lo2.status_code}  body={lo2.text!r}")

        random_tok = f"never-issued-{uuid4().hex}"
        lo3 = await c.post("/auth/logout", json={"refresh_token": random_tok})
        print(f"[step4] logout(unknown random token) -> {lo3.status_code}  body={lo3.text!r}")

        held = lo1.status_code == 204 and lo2.status_code == 204 and lo3.status_code == 204
        print(f"[verdict] logout-idempotent HELD={held} (all three == 204)")
