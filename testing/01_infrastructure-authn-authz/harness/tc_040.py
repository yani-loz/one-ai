...
# ======================= TC-IA-040 test logic =======================
# Refresh rotation is single-use: serial reuse of the OLD refresh token -> 401.

async def main() -> None:
    sfx = stamp()
    slug = f"token-{sfx}"
    admin_email = f"admin-{sfx}@token.example.com"
    print(f"[setup] namespace=token-{sfx} slug={slug} admin={admin_email}")
    ...
    async with _client() as c:
        ...
        lr = await login(c, admin_email, DEFAULT_PW)
        print(f"[step2] login -> {lr.status_code}")
        assert lr.status_code == 200, lr.text
        r0 = lr.json()["refresh_token"]
        print(f"[step2] R0(prefix)={r0[:12]}...")

        rr1 = await c.post("/auth/refresh", json={"refresh_token": r0})
        print(f"[step3] refresh(R0) -> {rr1.status_code}  body_keys={sorted(rr1.json().keys()) if rr1.headers.get('content-type','').startswith('application/json') else rr1.text}")
        r1 = rr1.json().get("refresh_token") if rr1.status_code == 200 else None
        print(f"[step3] R1(prefix)={r1[:12] if r1 else None}...  R1!=R0 -> {r1 != r0}")

        rr2 = await c.post("/auth/refresh", json={"refresh_token": r0})
        print(f"[step4] refresh(OLD R0 again) -> {rr2.status_code}  body={rr2.text}")

        held = rr1.status_code == 200 and r1 not in (None, r0) and rr2.status_code == 401
        print(f"[verdict] single-use HELD={held} (step3==200 & R1!=R0 & step4==401)")
