...
Role: ConnectorSyncCursor ORM model — the per-(org, connection, folder) incremental IMAP fetch
      cursor: which UIDVALIDITY + last UID we've durably stored, plus the parse-poison UIDs to step
      over. The runner UPSERTs it in the SAME per-batch transaction as the batch's email rows.
...
Key invariants:
  ...
  - `failed_uids` holds UIDs whose email permanently fails to parse/store, so the contiguous-prefix
    advance steps OVER them instead of wedging the folder forever (raw bytes are discarded on fail).
