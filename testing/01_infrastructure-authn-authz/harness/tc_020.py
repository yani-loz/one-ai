...
# ============================ TC-IA-020 test logic ============================
# Member's REAL token on GET /users and POST /users must be 403 PermissionDenied.
# The role gate (require_company_admin, dependencies.py:90-100) runs AFTER token
# verification, so a valid member token reaches the gate and must be rejected with 403.
...
async def main() -> None:
    ...
    async with _client() as c:
        ...
        org_slug = f"authz-{s}"
        admin_email = f"authz-admin-{s}@example.com"
        member_email = f"authz-member-{s}@example.com"

        ob = await onboard_org(
            c, plat, name=f"AuthZ Org {s}", slug=org_slug, admin_email=admin_email
        )
        print(f"[setup] onboard_org -> {ob.status_code} {ob.text}")

        admin_login = await login(c, admin_email, DEFAULT_PW)
        admin_token = admin_login.json()["access_token"]
        print(f"[setup] admin login -> {admin_login.status_code}")

        # Admin creates a real member in the same org.
        cu = await create_user(
            c, admin_token, email=member_email, full_name="Real Member", role="member"
        )
        print(f"[setup] create member -> {cu.status_code} {cu.text}")

        member_login = await login(c, member_email, DEFAULT_PW)
        member_token = member_login.json()["access_token"]
        print(f"[setup] member login -> {member_login.status_code}")

        # ATTACK 1: member token on GET /users
        g = await c.get("/users", headers=bearer(member_token))
        print(f"[attack] GET /users (member token) -> {g.status_code} {g.text}")

        # ATTACK 2: member token on POST /users (valid, schema-complete body)
        p = await c.post(
            "/users",
            headers=bearer(member_token),
            json={
                "email": f"authz-victim-{s}@example.com",
                "full_name": "Should Not Be Created",
                "role": "member",
                "password": DEFAULT_PW,
            },
        )
        print(f"[attack] POST /users (member token) -> {p.status_code} {p.text}")

        # Control: confirm the member token IS otherwise valid (passes auth on /auth/me).
        me = await c.get("/auth/me", headers=bearer(member_token))
        print(f"[control] GET /auth/me (member token) -> {me.status_code} {me.text}")
