...
async def main() -> None:
    s = stamp()
    slug = f"authn012-{s}"
    real_email = f"admin-authn012-{s}@oneai.dev"
    ghost_email = f"ghost-authn012-{s}@oneai.dev"  # never onboarded

    async with _client() as c:
        plat = await platform_login(c)
        ob = await onboard_org(
            c, plat, name=f"authn012 co-{s}", slug=slug, admin_email=real_email,
            admin_name="Authn012 Admin",
        )
        print(f"== onboard == {ob.status_code} real_admin={real_email}")

        a = await login(c, real_email, WRONG_PW)
        b = await login(c, ghost_email, WRONG_PW)

        print("== A: real email + wrong password ==")
        print(f"  status: {a.status_code}")
        print(f"  raw bytes: {a.content!r}")
        print("== B: ghost (never-registered) email + wrong password ==")
        print(f"  status: {b.status_code}")
        print(f"  raw bytes: {b.content!r}")

        status_eq = a.status_code == b.status_code
        bytes_eq = a.content == b.content
        json_eq = a.json() == b.json()
        print(f"status equal (A==B): {status_eq}")
        print(f"raw body bytes equal (A==B): {bytes_eq}")
        print(f"json equal (A==B): {json_eq}")

        ok = (
            a.status_code == 401 and b.status_code == 401
            and status_eq and bytes_eq and json_eq
