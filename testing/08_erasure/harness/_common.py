...
async def erase_org(c, plat_token, org_id, *, confirm_slug, reason="GDPR offboarding (test)", password=PLATFORM_PW):
    """Platform: POST /platform/orgs/{id}/erase (IRREVERSIBLE — only your own run-stamped orgs).

    `password` is the sudo-style re-auth required since commit 13da7fe (the admin re-enters their
    own password; verified before any delete). Defaults to the demo admin's password; for a
    forged-token test the admin-lookup fails first regardless of the password value."""
