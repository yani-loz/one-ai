...
# ─────────────────────────────────────────────────────────────────────────────
# TC-IA-052 — last-admin race: concurrent PATCH admin1->member + DELETE admin2
#
# Same non-atomic guard, mixed across both code paths at once: update_user (PATCH demote)
# and deactivate_user (DELETE). Both call _guard_last_admin excluding their own target;
# each sees the OTHER admin still active (count=1), both pass, one demotes + one
# deactivates, both commit -> 0 active company_admins. Proves the bug spans both mutating
# entry points simultaneously, not just like-with-like.
# ─────────────────────────────────────────────────────────────────────────────
...
async def main() -> None:
    ...
    async with _client() as c:
        ...
        for i in range(ITERATIONS):
            ...
            onb = await onboard_org(c, plat, name=f"Race052 {tag}", slug=slug, admin_email=a1_email)
            ...
            cr = await create_user(c, a1_token, email=a2_email, full_name="Admin Two", role="company_admin")
            ...
            # ── THE RACE: PATCH admin1->member  +  DELETE admin2  (mixed paths) ──
            async def patch_demote() -> httpx.Response:
                return await c.patch(f"/users/{admin1_id}", headers=bearer(a1_token), json={"role": "member"})

            async def delete_admin2() -> httpx.Response:
                return await c.delete(f"/users/{admin2_id}", headers=bearer(a1_token))

            res = await asyncio.gather(patch_demote(), delete_admin2(), return_exceptions=True)
            codes = tuple("EXC" if isinstance(r, BaseException) else r.status_code for r in res)
            # keep positional meaning: [0]=PATCH admin1, [1]=DELETE admin2
            pair_key = f"PATCH={codes[0]},DELETE={codes[1]}"
            ...
            if post_admins == 0:
                zero_admin_iters.append({"i": i, "org_id": org_id, "codes": codes, "active_admins_after": post_admins})

    print("=" * 70)
    print(f"TC-IA-052  run={run}  iterations={ITERATIONS}")
    print("=" * 70)
    ...
    for z in zero_admin_iters[:8]:
        print("   ", z)
    print("-" * 70)
    print("Sample org_ids for psql cross-check (first 8):")
    for oid in org_ids[:8]:
        print("   ", oid)
    print("=" * 70)
    print(
        f"VERDICT: {'GUARD BROKE — mixed PATCH+DELETE race fired' if zero_admin_iters else 'no lockout observed'}"
