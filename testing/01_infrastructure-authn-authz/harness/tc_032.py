...
# ======================================================================
# TC-IA-032 — admin A GET /users must return ONLY A's user emails and
# zero B emails. CONFIRMS-FIXED (list_by_org is org-scoped).
# ======================================================================
...
async def main() -> None:
    ...
    async with _client() as c:
        ...
        # Add a member to each org so each list has >1 row (richer surface for a leak).
        a_mem_email = f"{SUITE}-a-mem-{s}@{dom}"
        b_mem_email = f"{SUITE}-b-mem-{s}@{dom}"
        await create_user(c, a_tok, email=a_mem_email, full_name="A Member", role="member")
        await create_user(c, b_tok, email=b_mem_email, full_name="B Member", role="member")

        # admin A lists users.
        ra_list = await c.get("/users", headers=bearer(a_tok))
        print("A GET /users status:", ra_list.status_code)
        a_rows = ra_list.json()
        a_emails = sorted(u["email"] for u in a_rows)
        a_orgs = sorted({u["org_id"] for u in a_rows})
        print("A list emails:", a_emails)
        print("A list distinct org_ids:", a_orgs)

        b_emails_expected = {b_admin_email, b_mem_email}
        leaked = [e for e in a_emails if e in b_emails_expected]
        b_org_id = rb.json()["organization"]["id"]
        a_org_id = ra.json()["organization"]["id"]
        print("B emails that MUST NOT appear:", sorted(b_emails_expected))
        print("LEAKED B emails in A's list:", leaked, "(empty == pass)")
        print("A list contains B org_id?", b_org_id in a_orgs, "(False == pass)")
        print("A list org_ids == {A only}?", a_orgs == [a_org_id])
