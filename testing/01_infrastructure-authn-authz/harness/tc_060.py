...
# ───────────────────────── TC-IA-060 ─────────────────────────
# AUD-02 regression: a 73-byte ASCII password to POST /users must yield a CLEAN
# 422 from the BcryptPassword AfterValidator (byte-limit guard), NOT an opaque 500,
# and NO user is created. A 500 here REFUTES the AUD-02 fix.
...
async def main() -> None:
    s = stamp()
    org_slug = f"{SUITE}-060-{s}"
    admin_email = f"{SUITE}-060-admin-{s}@example.com"
    async with _client() as c:
        plat = await platform_login(c)
        ob = await onboard_org(
            c, plat, name=f"IV060 {s}", slug=org_slug, admin_email=admin_email
        )
        ...
        pw_73 = "A" * 73  # 73 ASCII bytes — 1 over bcrypt's 72-byte hard limit
        print(f"[probe] password length: chars={len(pw_73)} bytes={len(pw_73.encode('utf-8'))}")
        target_email = f"{SUITE}-060-victim-{s}@example.com"
        r = await create_user(
            c, admin_token, email=target_email, full_name="Pw73 Probe",
            role="member", password=pw_73,
        )
        print(f"[create_user pw=73B] status={r.status_code}")
        print(f"[create_user pw=73B] body={r.text}")

        # Confirm no user was created under that email (list returns own-org only).
        lst = await c.get("/users", headers=bearer(admin_token))
        created_emails = [u["email"] for u in lst.json()] if lst.status_code == 200 else []
        print(f"[verify] list status={lst.status_code} emails={created_emails}")
        print(f"[verify] victim_present={target_email in created_emails}")
