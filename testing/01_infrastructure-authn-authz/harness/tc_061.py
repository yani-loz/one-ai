...
# ───────────────────────── TC-IA-061 ─────────────────────────
# Multibyte password: UNDER 72 CHARACTERS but OVER 72 BYTES (30 emoji ~= 120 bytes).
# Must hit the byte-limit AfterValidator -> clean 422 (not 500), NO user created.
# AUD-02 regression for the multibyte axis.
...
async def main() -> None:
    s = stamp()
    org_slug = f"{SUITE}-061-{s}"
    admin_email = f"{SUITE}-061-admin-{s}@example.com"
    async with _client() as c:
        plat = await platform_login(c)
        ob = await onboard_org(c, plat, name=f"IV061 {s}", slug=org_slug, admin_email=admin_email)
        ...
        pw_emoji = "\U0001F600" * 30  # 30 emoji, each 4 UTF-8 bytes = 120 bytes, 30 chars
        n_chars = len(pw_emoji)
        n_bytes = len(pw_emoji.encode("utf-8"))
        print(f"[probe] password: chars={n_chars} bytes={n_bytes} (chars<72, bytes>72)")
        target_email = f"{SUITE}-061-victim-{s}@example.com"
        r = await create_user(c, admin_token, email=target_email, full_name="Emoji Pw",
                              role="member", password=pw_emoji)
        print(f"[create_user multibyte>72B] status={r.status_code}")
        print(f"[create_user multibyte>72B] body={r.text}")

        lst = await c.get("/users", headers=bearer(admin_token))
        emails = [u["email"] for u in lst.json()] if lst.status_code == 200 else []
        print(f"[verify] list status={lst.status_code} victim_present={target_email in emails}")
