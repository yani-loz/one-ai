...
## Expected result
What SHOULD happen — the exact status codes / body shape / side effects the contract requires.
...
<Did the defense hold or break? If it broke: severity, blast radius, and the precise
code path (`file:line`) responsible. If it confirms/refutes a prior audit claim, say which.>
