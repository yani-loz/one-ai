...
# ======================= TC-IA-043 test logic =======================
# Refresh AFTER logout -> 401 (logout durably revoked the refresh token).
...
async def main() -> None:
    ...
    async with _client() as c:
        ...
        lo = await c.post("/auth/logout", json={"refresh_token": r0})
        print(f"[step2] logout(R0) -> {lo.status_code}  body={lo.text!r}")

        rr = await c.post("/auth/refresh", json={"refresh_token": r0})
        print(f"[step3] refresh(R0 after logout) -> {rr.status_code}  body={rr.text}")

        held = lo.status_code == 204 and rr.status_code == 401
        print(f"[verdict] refresh-after-logout HELD={held} (logout==204 & refresh==401)")
