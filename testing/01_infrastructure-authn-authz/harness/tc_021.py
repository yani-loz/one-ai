...
# ============================ TC-IA-021 test logic ============================
# Positive control: a real company_admin token on GET /users returns 200 with a list
# (the admin's own org users only). Proves the role gate ALLOWS the admin through.
...
async def main() -> None:
    ...
    async with _client() as c:
        ...
        g = await c.get("/users", headers=bearer(admin_token))
        print(f"[control] GET /users (admin token) -> {g.status_code} {g.text}")
        if g.status_code == 200:
            body = g.json()
            print(f"[control] user_count={len(body)}; roles={[u.get('role') for u in body]}")
