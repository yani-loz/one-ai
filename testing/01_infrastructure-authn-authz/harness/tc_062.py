...
# ───────────────────────── TC-IA-062 ─────────────────────────
# Password length boundaries on POST /users:
#   - exactly 72 ASCII bytes  -> 201 (upper bound inclusive, hashes fine)
#   - exactly 8 chars         -> 201 (min_length lower bound inclusive)
...
async def main() -> None:
    s = stamp()
    org_slug = f"{SUITE}-062-{s}"
    admin_email = f"{SUITE}-062-admin-{s}@example.com"
    async with _client() as c:
        plat = await platform_login(c)
        ob = await onboard_org(c, plat, name=f"IV062 {s}", slug=org_slug, admin_email=admin_email)
        ...
        cases = [
            ("72-byte-ASCII", "A" * 72, 201),
            ("8-char-min",    "Abcd1234", 201),
            ("7-char-too-short", "Abc1234", 422),
        ]
        for label, pw, expected in cases:
            email = f"{SUITE}-062-{label}-{s}@example.com"
            r = await create_user(c, admin_token, email=email, full_name=f"Boundary {label}",
                                  role="member", password=pw)
            print(f"[{label}] pw_chars={len(pw)} pw_bytes={len(pw.encode('utf-8'))} "
                  f"status={r.status_code} expected={expected} match={r.status_code == expected}")
            if r.status_code not in (201, 422):
                print(f"[{label}] body={r.text}")
