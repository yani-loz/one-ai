...
# ============================ TC-IA-022 test logic ============================
# A REAL company token on GET /platform/orgs and POST /platform/orgs must be 401:
# decode_access_token(aud='platform') rejects a token minted with aud='company'
# (tokens.py:81 audience= enforced). The platform-admin gate is never reached.
...
async def main() -> None:
    ...
    async with _client() as c:
        ...
        company_token = admin_login.json()["access_token"]
        print(f"[setup] company admin login -> {admin_login.status_code} (aud=company)")

        g = await c.get("/platform/orgs", headers=bearer(company_token))
        print(f"[attack] GET /platform/orgs (company token) -> {g.status_code} {g.text}")

        p = await c.post(
            "/platform/orgs",
            headers=bearer(company_token),
            json={
                "org_name": f"Smuggled Org {s}",
                "org_slug": f"authz-smuggle-{s}",
                "admin_email": f"authz-smuggle-{s}@example.com",
                "admin_full_name": "Smuggle Admin",
                "admin_password": DEFAULT_PW,
            },
        )
        print(f"[attack] POST /platform/orgs (company token) -> {p.status_code} {p.text}")
